...
typedef bool target_stats_clients_cb_t(ds_dlist_t *client_list, void *ctx, int status);
typedef bool target_stats_survey_cb_t(ds_dlist_t *survey_list, void *survey_ctx, int status);
typedef bool target_scan_cb_t(void *scan_ctx, int status);
                              ...
                              void *client_ctx)
{
    ...
    util_qca_set_int(radio_cfg->phy_name, "flush_rate_stats", 1);
...
bool target_stats_vif_get(dpp_vif_record_t *record)
{
    return ioctl80211_stats_vif_get(record);
}
