...
util_net_get_phy_macaddr_str(const char *phy, char *buf, int len)
{
    char path[128];
    int err;
    snprintf(path, sizeof(path), "/tmp/%s", phy);
        ...
        if (kconfig_enabled(CONFIG_PLATFORM_IS_MTK)) {
            if (wiphy_info) {
                ...
                if (strstr(wiphy_info->band, "2.4G") || strstr(wiphy_info->band, "5G")) {
                    STRSCPY(hapd->htcaps, "[LDPC][[TX-STBC][RX-STBC1][SHORT-GI-20][SHORT-GI-40]");
                }

                // set vhtcaps for 5G and 6G ap config
                if ((hapd->ieee80211ac) && strstr(wiphy_info->band, "5G")) {
                    STRSCPY(hapd->vhtcaps, "[RXLDPC][TX-STBC-2BY1][RX-STBC-1][MAX-A-MPDU-LEN-EXP7][SHORT-GI-80][SHORT-GI-160][VHT160]");
                }
	...
	if(hapd->ieee80211ax == 1) {
      ...
      if ( (!strcmp(rconf->freq_band, SCHEMA_CONSTS_RADIO_TYPE_STR_5G)) ||
           (!strcmp(rconf->freq_band, SCHEMA_CONSTS_RADIO_TYPE_STR_6G))
            ...
            csnprintf(&buf, &len, "he_bss_color=128\n");
      }
	}
...
rconf_chan:
    if (util_lookup_rconf_by_ifname(&rconf, phy)) {
        ...
        strcpy(htmode, rconf.ht_mode);
        return true;
    }

    LOGT("%s: get ht mode fail", phy);
    return false;
}
                         ...
                         const struct schema_Wifi_Radio_Config_flags *changed)
{
    ...
    if (kconfig_enabled(CONFIG_TARGET_USE_ANTENNA_AS_CHAIN) &&
        ...
        /* OpenSync not intend to chagne RX chain mask and could simply treat
         * TX/RX chainmask are the same if using antenna mask as chain mask to
         * maximum compatibility if the wifi driver not support unequal antenna
         * mask, like MediaTek 11AX series.
         */
...
static bool
util_vif_enabled(const char *vif)
{
    char state[BFR_SIZE_64];
    char opmode[256];
    bool ret;
        ...
        if (os_nif_is_running(vif, &running) && running) {
            ...
            if (ret == true) {
                if (strcmp(state, "ENABLED") == 0) {
                    ...
                    LOGI("%s: VIF is fakely true for hostapd tranistion state", vif);
                    return true;
                }
            } else {
                LOGE("%s: hostap ap get status fail", vif);
                return false;
            }
        }
        ...
        if (os_nif_is_running(vif, &running) && running) {
            ...
            if (ret == true) {
                if (strcmp(state, "COMPLETED") == 0) {
                    ...
                    LOGI("%s: VIF is fakely true for wpa tranistion state", vif);
                    return true;
                }
            } else {
                LOGE("%s: hostap sta get status fail", vif);
                return false;
            }
        }
    } else {
        LOGE("%s: unexpcted vif mode %s", vif, opmode);
        return false;
    }
}
