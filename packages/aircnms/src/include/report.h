...
/* VIF Record (combined) */
typedef struct {
    vif_info_record_t info;   // VIF info
    vif_stats_record_t stats; // VIF stats
} vif_record_t;
...
typedef struct {
    ...
    uint32_t is_connected;          // Connection status
    char client_type[16];           // Client type (e.g., "wireless")
    char osinfo[64];                // OS information
...
typedef struct {
    ...
    sta_info_t info;        // Station info
    sta_stats_t stats;      // Station stats
} client_record_t;
