...
#define SOCKET_PATH "/tmp/aircnms/cm.sock"
...
static void client_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (revents & EV_READ) {
        ...
        cm_item_t *ci = CALLOC(1, sizeof(cm_item_t));
        if (!ci) {
            unixcomm_log_error("Failed to allocate cm_item_t");
            goto cleanup_message;
        }
        // Fill request metadata
        //memcpy(&ci->req, &message.request, sizeof(cm_request_t));
        ci->req.data_type = message.request.data_type; 
        if (message.topic && message.request.topic_len) {
            ci->topic = MALLOC(message.request.topic_len + 1);
            if (!ci->topic) {
                unixcomm_log_error("Failed to allocate topic");
                cm_queue_item_free(ci);
                goto cleanup_message;
            }
            memcpy(ci->topic, message.topic, message.request.topic_len);
            ci->topic[message.request.topic_len] = '\0';
        }
        if (message.data && message.data_size) {
            ci->buf = MALLOC(message.data_size);
            if (!ci->buf) {
                unixcomm_log_error("Failed to allocate data buffer");
                cm_queue_item_free(ci);
                goto cleanup_message;
            }
            memcpy(ci->buf, message.data, message.data_size);
            ci->size = message.data_size;
        }
        {
            cm_response_t res = {0};
            if (!cm_queue_put(&ci, &res)) {
                unixcomm_log_error("Queue put failed: error=%u", res.error);
                if (ci) cm_queue_item_free(ci);
            }
        }
cleanup_message:
...
bool cm_unixcomm_server_init(void) {
    // Store event loop
    ...
    config.target_process = UNIXCOMM_PROCESS_CM; 
...
void cm_unixcomm_server_cleanup(void) {
    if (!g_server.running) return;
