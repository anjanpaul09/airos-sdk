...
#define SOCKET_PATH "/tmp/aircnms/cgw.sock"
...
static void client_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (revents & EV_READ) {
        ...
        if (!qi) {
            unixcomm_log_error("Failed to allocate cgw_item_t");
            goto cleanup_message;
        }
        ...
        if (message.topic && message.request.topic_len) {
            ...
            memcpy(qi->topic, message.topic, message.request.topic_len);
            qi->topic[message.request.topic_len] = '\0';
        }
        if (message.data && message.data_size) {
            qi->buf = MALLOC(message.data_size);
            if (!qi->buf) {
                unixcomm_log_error("Failed to allocate data buffer");
                cgw_queue_item_free(qi);
                goto cleanup_message;
            }
            memcpy(qi->buf, message.data, message.data_size);
            qi->size = message.data_size;
        }
        {
...
bool cgw_unixcomm_server_init(void) {
    ...
    if (!unixcomm_init(&global_config)) {
        LOG(ERR, "Failed to initialize unixcomm");
        return false;
    }
    ...
    config.target_process = UNIXCOMM_PROCESS_QM; 
    ...
    LOG(INFO, "Server initialized on %s", SOCKET_PATH);
    return true;
}
