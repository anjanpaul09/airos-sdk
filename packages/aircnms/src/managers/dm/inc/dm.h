...
{
    uint32_t data_size;
    uint32_t data_type; // dm_data_type_t
} dm_request_t;
...
//typedef struct { uint32_t response; uint32_t error; uint32_t qdrop; } dm_response_t;
//enum { DM_DATA_LOG = 3, DM_DATA_CMD = 7 };
enum { DM_RESPONSE_ERROR = 0 };
enum { DM_ERROR_QUEUE = 103 };
