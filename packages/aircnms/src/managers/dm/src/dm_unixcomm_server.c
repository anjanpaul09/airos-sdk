...
#define SOCKET_PATH "/tmp/aircnms/dm.sock"
...
static void client_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (revents & EV_READ) {
        ...
        unixcomm_log_info("Received message, data size: %zu data = %s ", message.data_size, message.data);
        // Enqueue into CM queue and signal MQTT worker
        dm_item_t *di = CALLOC(1, sizeof(dm_item_t));
        if (!di) {
            unixcomm_log_error("Failed to allocate dm_item_t");
            goto cleanup_message;
        }
        // Fill request metadata
        //memcpy(&ci->req, &message.request, sizeof(cm_request_t));
        di->req.data_type = message.request.data_type; 
        if (message.topic && message.request.topic_len) {
            di->topic = MALLOC(message.request.topic_len + 1);
            if (!di->topic) {
                unixcomm_log_error("Failed to allocate topic");
                dm_queue_item_free(di);
                goto cleanup_message;
            }
            memcpy(di->topic, message.topic, message.request.topic_len);
            di->topic[message.request.topic_len] = '\0';
        }
        if (message.data && message.data_size) {
            di->buf = MALLOC(message.data_size);
            if (!di->buf) {
                unixcomm_log_error("Failed to allocate data buffer");
                dm_queue_item_free(di);
                goto cleanup_message;
            }
            memcpy(di->buf, message.data, message.data_size);
            di->size = message.data_size;
        }
        {
            dm_process_msg(di);
            dm_response_t res = {0};
            if (!dm_queue_put(&di, &res)) {
                unixcomm_log_error("Queue put failed: error=%u", res.error);
                if (di) dm_queue_item_free(di);
            }
        }
cleanup_message:
...
bool dm_unixcomm_server_init(void) {
    // Store event loop
    ...
    config.target_process = UNIXCOMM_PROCESS_DM; 
...
void dm_unixcomm_server_cleanup(void) {
    if (!g_server.running) return;
