...
int main(int argc, char **argv)
{
    ...
    // Initialize unixcomm client for QM communication
    //if (!sm_unixcomm_client_init()) {
      //  LOG(ERR, "Failed to initialize unixcomm client");
        //return -1;
    //}
    ...
    if (!sm_unixcomm_server_init()) {
        LOG(ERR, "DM: Failed to initialize unixcomm server");
        return -1;
    }
    ...
    ev_run(EV_DEFAULT, 0);
    ev_default_destroy();
    //sm_unixcomm_client_cleanup();
    sm_unixcomm_server_cleanup();
