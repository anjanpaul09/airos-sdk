...
#define SOCKET_PATH "/tmp/aircnms/netconf.sock"
...
static void client_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (revents & EV_READ) {
        ...
        netconf_item_t *ci = CALLOC(1, sizeof(netconf_item_t));
        if (!ci) {
            unixcomm_log_error("Failed to allocate netconf_item_t");
            goto cleanup_message;
        }
        // Fill request metadata
        //memcpy(&ci->req, &message.request, sizeof(netconf_request_t));
        ...
        }
        {
            ...
            if (!netconf_queue_put(&ci, &res)) {
                unixcomm_log_error("Queue put failed: error=%u", res.error);
                if (ci) netconf_queue_item_free(ci);
            }
        }
cleanup_message:
