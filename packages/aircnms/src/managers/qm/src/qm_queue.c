...
pthread_mutex_t g_qm_queue_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t  g_qm_queue_cond  = PTHREAD_COND_INITIALIZER;
...
bool qm_queue_append_item(qm_item_t **qitem, qm_response_t *res)
{
    ...
    //pthread_cond_signal(&g_qm_queue_cond);
    qm_mqtt_signal_new_item(); // wake worker
    pthread_mutex_unlock(&g_qm_queue_mutex);
    // take ownership
    *qitem = NULL;
    return true;
...
bool qm_queue_put(qm_item_t **qitem, qm_response_t *res)
{
    ...
    if ((*qitem)->req.data_type == DATA_LOG) {
        ...
        *qitem = NULL;
        ...
        result = qm_send_config_cloud(*qitem);
    } else if ((*qitem)->req.data_type == DATA_EVENT) {
        result = qm_send_event_cloud(*qitem);
