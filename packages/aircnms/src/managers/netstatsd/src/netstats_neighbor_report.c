...
typedef struct
{
    ...
    uint64_t                        report_ts;
} netstats_neighbor_ctx_t;

static netstats_neighbor_ctx_t              g_netstats_neighbor_ctx;

static bool netstats_neighbor_timer_set(ev_timer *timer, bool enable)
{
    if (enable) {
...
static
bool netstats_neighbor_report_timer_restart(
        ev_timer                   *timer)
{
    netstats_neighbor_ctx_t                   *neighbor_ctx;
    ...
    neighbor_ctx =
        (netstats_neighbor_ctx_t *) timer->data;
    ...
    if (request_ctx->reporting_count) {
        ...
        if (0 == request_ctx->reporting_count) {
            netstats_neighbor_timer_set(timer, false);
...
static void netstats_neighbor_report_stats(netstats_neighbor_ctx_t *neighbor_ctx)
{
    bool                            status;
    netstats_request_t               *request_ctx = &neighbor_ctx->request;
    neighbor_report_data_t           *report_ctx = &neighbor_ctx->report;
    ev_timer                       *report_timer =
        &neighbor_ctx->report_timer;

    netstats_neighbor_report_timer_restart(report_timer);
...
static void netstats_neighbor_report(EV_P_ ev_timer *w, int revents)
{
    netstats_neighbor_report_stats(w->data);
    //ev_timer_stop(EV_A_ w); // Stops the timer
}

static void
netstats_nlev_neighbor_handle_signal(struct ev_loop *loop, ev_signal *w, int revents)
{
    netstats_neighbor_ctx_t *neighbor_ctx = (netstats_neighbor_ctx_t *)w->data;
...
{
    netstats_neighbor_ctx_t             *neighbor_ctx = NULL;
    neighbor_ctx = &g_netstats_neighbor_ctx;
    netstats_request_t          *request_ctx = &neighbor_ctx->request;
    neighbor_report_data_t        *report_ctx = &neighbor_ctx->report;
    ev_timer                    *report_timer = &neighbor_ctx->report_timer;
    ...
    if (!neighbor_ctx->initialized) {
        ...
        ev_init(report_timer, netstats_neighbor_report);
        report_timer->data = neighbor_ctx;

        ev_signal_init(ev_sigusr, netstats_nlev_neighbor_handle_signal, SIGUSR2);
    ...
    netstats_neighbor_timer_set(report_timer, false);
    ...
    if (request_ctx->reporting_interval) {
        ...
        netstats_neighbor_timer_set(report_timer, true);
