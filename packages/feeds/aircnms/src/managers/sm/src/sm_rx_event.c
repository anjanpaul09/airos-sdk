...
{
    sm_request_t *req = &qi->req;
    sm_response_t res;

    LOG(TRACE, "%s", __FUNCTION__);
    // enqueue
    sm_res_init(&res, req);
    if (req->cmd == SM_CMD_SEND && req->data_size) {
        if (req->flags & SM_REQ_FLAG_SEND_DIRECT) {
            //sm_mqtt_send_message(qi, &res);
        } else {
...
bool sm_async_handle_req(sm_async_ctx_t *ctx)
{
    ...
    for (;;) {
        ...
        ret = sm_conn_parse_req(ctx->buf, ctx->size, &qi->req, &qi->topic, &qi->buf, &complete);
        if (ret && complete) {
            // shift consumed data in ctx buf
            size = sizeof(qi->req) + qi->req.topic_len + qi->req.data_size;
            sm_ctx_shift_buf(ctx, size);
            // enqueue
            qi->size = qi->req.data_size;
            sm_enqueue_and_reply(ctx->fd, qi);
        } else {
...
void sm_sock_callback(struct ev_loop *ev, struct ev_io *io, int event)
{
    ...
    if (event & EV_READ)
    {
        if (!sm_conn_accept(g_sm_sock, &fd)) return;
        sm_async_new(fd);
    }
}
...
bool sm_rx_event_init()
{
    ...
    if (!sm_conn_server(&g_sm_sock)) {
        return false;
    }

    ev_io_init(&g_sm_sock_ev, sm_sock_callback, g_sm_sock, EV_READ);
    //g_sm_sock_ev.data = ...;
    ev_io_start(EV_DEFAULT, &g_sm_sock_ev);
