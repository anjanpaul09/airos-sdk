...
bool cm_process_vif_list(json_t *vif_list)
{
    bool ret;
    int n_vif = 0;
    //char record_id[12];
...
bool cm_process_radio_list(json_t *radio_list)
{
    bool ret;
    int n_radio = 0;
    char record_id[12];
...
bool cm_process_blacklist(json_t *blackList)
{
    char tmp_mac[32];
    char type[16];
        ...
        for (size_t i = 0; i < json_array_size(add_list); i++) {
            ...
            if (json_is_string(mac)) {
                ...
                if (strncmp(type, "ssid", 4) == 0) {
                    ...
                    cm_handle_add_blacklist_ssid(tmp_mac, ssid);
                } else {
                    cm_handle_add_blacklist(tmp_mac);
                }
            }
        }
    }
    ...
    if (json_is_array(remove_list)) {
        ...
        for (size_t i = 0; i < json_array_size(remove_list); i++) {
            ...
            if (json_is_string(mac)) {
                ...
                memset(tmp_mac, 0, sizeof(tmp_mac));
                strcpy(tmp_mac, json_string_value(mac));
                cm_handle_remove_blacklist(tmp_mac);
            }
        }
    }
    return true;
...
int cm_process_whitelist(json_t *whiteList)
{
    char tmp_mac[32];
    char type[16];
        ...
        for (size_t i = 0; i < json_array_size(add_list); i++) {
            ...
            if (json_is_string(mac)) {
                ...
                if (strncmp(type, "ssid", 4) == 0) {
                    ...
                    cm_handle_add_whitelist_ssid(tmp_mac, ssid);
                } else {
                    cm_handle_add_whitelist(tmp_mac);
                }
            }
        }
    }
    ...
    if (json_is_array(remove_list)) {
        ...
        for (size_t i = 0; i < json_array_size(remove_list); i++) {
            ...
            if (json_is_string(mac)) {
                ...
                memset(tmp_mac, 0, sizeof(tmp_mac));
                strcpy(tmp_mac, json_string_value(mac));
                cm_handle_remove_whitelist(tmp_mac);
            }
        }
    }
    return true;
...
int cm_process_nat_config(json_t *nat_config)
{
    int ret;
    nat_config_t config;
    ...
    ret = cm_handle_nat_config(&config);
    return ret;
}
...
int cm_process_acl_msg(char *buf)
{
    ...
    if (blackList) {
        cm_process_blacklist(blackList);
    }

    json_t *whiteList = json_object_get(root, "whiteList");
    if (whiteList) {
        cm_process_whitelist(whiteList);
    }

    json_decref(root);
    
#ifdef CONFIG_PLATFORM_MTK_JEDI
    cm_check_wifi_config();
#endif
