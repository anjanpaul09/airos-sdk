...
void cm_add_acl_entry(const char *profile, int index, const char *mac_address, acl_type type) 
{
...
void cm_del_acl_entry(const char *profile, int index, const char *mac_address, acl_type type) 
{
    char cmd[MAX_CMD_LEN], result[MAX_RESULT_LEN], new_list[MAX_RESULT_LEN] = "";
...
void cm_clear_acl(const char *profile, acl_type type) {
    char cmd[256];
...
int cm_handle_add_blacklist(char *mac)
{
    ...
    if (get_ifname_by_mac(mac, ifname, sizeof(ifname))) {
        ...
        cm_add_acl_entry(wifi_profile_ra, ifindex, mac, ACL_BLACKLIST); 
        
        const char *wifi_profile_rax = RAX_PROFILE;
        cm_add_acl_entry(wifi_profile_rax, ifindex, mac, ACL_BLACKLIST); 
    }
...
int cm_handle_remove_blacklist(char *mac)
{
    for (int i = 0; i < 3; i++) {
        ...
        cm_del_acl_entry(wifi_profile, i, mac, ACL_DEFAULT); 
    }
    for (int i = 0; i < 3; i++) {
...
int cm_handle_add_whitelist(char *mac)
{
    ...
    if (get_ifname_by_mac(mac, ifname, sizeof(ifname))) {
        ...
        const char *wifi_profile_ra = RA_PROFILE;
        cm_add_acl_entry(wifi_profile_ra, ifindex, mac, ACL_WHITELIST); 
        
        const char *wifi_profile_rax = RAX_PROFILE;
        cm_add_acl_entry(wifi_profile_rax, ifindex, mac, ACL_WHITELIST); 
