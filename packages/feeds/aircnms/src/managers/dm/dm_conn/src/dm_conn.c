...
#define DM_SOCK_DIR "/tmp/aircnms/"
#define DM_SOCK_FILENAME DM_SOCK_DIR"dm.sock"
#define DM_SOCK_MAX_PENDING 10
#define DM_COMPACT_SEND_SIZE (64*1024)

static double dm_conn_default_timeout = DM_CONN_DEFAULT_TIMEOUT;
...
bool dm_conn_server(int *pfd)
{
    ...
    char *path = DM_SOCK_FILENAME;
    int fd;

    mkdir(DM_SOCK_DIR, 0755);
    ...
    *pfd = -1;
    ...
    if (listen(fd, DM_SOCK_MAX_PENDING) < 0) {
        LOG(ERR, "listen");
        close(fd);
        return false;
    }
...
{
    dm_conn_default_timeout = timeout;
}
...
bool dm_conn_client(int *pfd)
{
    ...
    *pfd = -1;
    ...
    if (dm_conn_default_timeout > 0) {
        if (!dm_conn_set_fd_timeout(fd, dm_conn_default_timeout)) {
            close(fd);
            return false;
        }
    }
...
void dm_req_init(dm_request_t *req)
{
    ...
    memcpy(req->tag, DM_REQUEST_TAG, sizeof(req->tag));
    STRSCPY(req->sender, log_get_name());
    req->ver = DM_REQUEST_VER;
    req->seq = seq;
    seq++;
}
...
{
    return (memcmp(req->tag, DM_REQUEST_TAG, sizeof(req->tag)) == 0)
            && (req->ver == DM_REQUEST_VER);
}
...
void dm_res_init(dm_response_t *res, dm_request_t *req)
{
    ...
    memcpy(res->tag, DM_RESPONSE_TAG, sizeof(res->tag));
    res->ver = DM_RESPONSE_VER;
    res->seq = req->seq;
    switch (req->cmd) {
        case DM_CMD_STATUS:
            res->response = DM_RESPONSE_STATUS;
            break;
        case DM_CMD_SEND:
            res->response = DM_RESPONSE_RECEIVED;
            break;
        default:
            break;
    }
...
{
    return (memcmp(res->tag, DM_RESPONSE_TAG, sizeof(res->tag)) == 0)
            && (res->ver == DM_RESPONSE_VER);
}
...
bool dm_conn_get_status(dm_response_t *res)
{
    dm_request_t req;
    dm_req_init(&req);
    req.cmd = DM_CMD_STATUS;
    return dm_conn_send_req(&req, NULL, NULL, 0, res);
}

char *dm_data_type_str(enum dm_req_data_type type)
{
    switch (type) {
        case DM_DATA_RAW:   return "raw";
        case DM_DATA_TEXT:  return "bs";
        case DM_DATA_STATS: return "stats";
        case DM_DATA_LOG:   return "log";
...
{
    switch (x) {
        case DM_RESPONSE_ERROR:    return "error";
        case DM_RESPONSE_STATUS:   return "status";
        case DM_RESPONSE_RECEIVED: return "ok";
        case DM_RESPONSE_IGNORED:  return "ignored";
    }
    return "unk";
}
...
{
    switch (x) {
        case DM_ERROR_NONE:    return "";
        case DM_ERROR_GENERAL: return "error";
        case DM_ERROR_CONNECT: return "connect";
        case DM_ERROR_INVALID: return "invalid";
        case DM_ERROR_QUEUE:   return "queue";
        case DM_ERROR_SEND:    return "send";
    }
    return "unk";
}
...
{
    switch (x) {
        case DM_CONN_STATUS_NO_CONF:      return "no-conf";
        case DM_CONN_STATUS_DISCONNECTED: return "diconnected";
        case DM_CONN_STATUS_CONNECTED:    return "connected";
    }
    return "unk";
}
...
bool dm_conn_open_fd(int *fd, dm_response_t *res)
{
    ...
    if (!res) res = &res1;
    MEMZERO(*res);
    if (!dm_conn_client(fd)) {
        res->error = DM_ERROR_CONNECT;
        LOG(ERROR, "connecting to dm");
        return false;
    }
    return true;
}
...
bool dm_conn_send_fd(int fd, dm_request_t *req, char *topic, void *data, int data_size, dm_response_t *res)
{
    ...
    if (!dm_conn_write_req(fd, req, topic, data, data_size)) {
        res->error = DM_ERROR_CONNECT;
        goto out;
    }
    if (!(req->flags & DM_REQ_FLAG_NO_RESPONSE)) {
        if (!dm_conn_read_res(fd, res)) {
            goto out;
        }
    } else {
        res->response = DM_RESPONSE_IGNORED;
    }
    result = true;
out:
    if (!result || res->response == DM_RESPONSE_ERROR) {
        // on either error set both return value and response type to error
        result = false;
        res->response = DM_RESPONSE_ERROR;
        if (!res->error) res->error = DM_ERROR_GENERAL;
        if (req->cmd != DM_CMD_STATUS) {
    ...
    if (result && req->cmd == DM_CMD_SEND) {
        LOG(DEBUG, "Sent message to DM (size: %d type: %s)",
                data_size, dm_data_type_str(req->data_type));
    }
...
bool dm_conn_send_req(dm_request_t *req, char *topic, void *data, int data_size, dm_response_t *res)
{
    int fd = -1;
    bool result = false;
    if (!dm_conn_open_fd(&fd, res)) {
        return false;
    }
    result = dm_conn_send_fd(fd, req, topic, data, data_size, res);
    close(fd);
    return result;
}
        ...
        dm_response_t *res)
{
    ...
    req.flags = flags;
    return dm_conn_send_req(&req, topic, data, data_size, res);
}

bool dm_conn_send_raw(char *topic, void *data, int data_size, dm_response_t *res)
{
    return dm_conn_send_custom(
            DM_DATA_RAW, DM_REQ_COMPRESS_DISABLE, 0,
            topic, data, data_size, res);
}

bool dm_conn_send_direct(dm_compress_t compress, char *topic,
        void *data, int data_size, dm_response_t *res)
{
    return dm_conn_send_custom(
            DM_DATA_RAW, compress,
            DM_REQ_FLAG_SEND_DIRECT,
            topic, data, data_size, res);
}
...
bool dm_conn_send_initial(void *data, int data_size, dm_response_t *res)
{
    dm_request_t req;
    dm_req_init(&req);
    req.cmd = DM_CMD_SEND;
    req.data_type = DM_DATA_INI;
    req.compress = DM_REQ_COMPRESS_IF_CFG;
    return dm_conn_send_req(&req, NULL, data, data_size, res);
}
// streaming api
...
bool dm_conn_open(dm_conn_t *qc)
{
    ...
    qc->init = true;
    qc->fd = -1;
    return dm_conn_open_fd(&qc->fd, &qc->res);
}

bool dm_conn_reopen(dm_conn_t *qc)
{
...
bool dm_conn_check_reconnect(dm_conn_t *qc)
{
    if (!qc->init) return false;
    if (qc->fd < 0) {
        // fd not open - reopen
        return dm_conn_reopen(qc);
    }
...
bool dm_conn_send_stream(dm_conn_t *qc, dm_request_t *req, char *topic, void *data, int data_size, dm_response_t *res)
{
    bool result = false;
    if (!qc || !qc->init) {
        if (res) MEMZERO(*res);
    ...
    if (!dm_conn_check_reconnect(qc)) {
        return false;
    }
    // send
    result = dm_conn_send_fd(qc->fd, req, topic, data, data_size, &qc->res);
    if (!result && (qc->res.error == DM_ERROR_CONNECT)) {
        // on connection error try to reconnect and resend
        if (!dm_conn_reopen(qc)) goto out;
        result = dm_conn_send_fd(qc->fd, req, topic, data, data_size, &qc->res);
    }
out:
    if (res) { *res = qc->res; }
...
dm_conn_t dm_conn_log_handle;

bool dm_conn_send_log(char *msg, dm_response_t *res)
{
    dm_conn_t *qc = &dm_conn_log_handle;
    if (!qc->init) {
        dm_conn_open(qc);
    }
    dm_request_t req;
    ...
    req.flags = DM_REQ_FLAG_NO_RESPONSE;
    return dm_conn_send_stream(qc, &req, NULL, msg, strlen(msg), res);
}
