...
void update_vif_top_domains(const char *domain_name) 
{
    ...
    for (i = 0; i < MAX_DOMAINS; i++) {
        if (strcmp(vif->top_domains[i].domain, domain_name) == 0) {
            vif->top_domains[i].count++;
            spin_unlock(&vif->domain_lock);
            return;
        }
        if (vif->top_domains[i].count < min_count) {
            min_count = vif->top_domains[i].count;
            min_index = i;
        }
    }

    // If the domain is not in the list and there's space, add it
    if (min_index != -1 && vif->top_domains[min_index].count == 0) {
        strncpy(vif->top_domains[min_index].domain, domain_name, MAX_DOMAIN_NAME_LEN - 1);
        vif->top_domains[min_index].domain[MAX_DOMAIN_NAME_LEN - 1] = '\0';
        vif->top_domains[min_index].count = 1;
        spin_unlock(&vif->domain_lock);
        return;
    }
