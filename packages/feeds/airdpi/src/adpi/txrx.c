...
int is_blocked_ip(struct air_vif *vif, __be32 ip) {
...
struct client_node *client_reg_table_alloc(char *macaddr)
{
    ...
    hash = CLIENT_HASH(macaddr);
    LIST_INSERT_HEAD(&vif->nc.client_hash[hash], cn, nh);
    TAILQ_INSERT_TAIL(&vif->nc.client_list, cn, nl);
...
static struct wlan_sta *sta_table_lookup(uint8_t *macaddr, int dir, uint8_t ifindex)
{
...
#if 0
void packet_counter_reset(struct timer_list *timer) 
{
    printk(KERN_INFO "rx len : %lu\n", rx_total_len);
    printk(KERN_INFO "tx len: %lu\n", tx_total_len);
...
unsigned int air_ingress_hook(struct sk_buff *skb, const struct nf_hook_state* state)
{
    ...
    //if (is_blocked_ip(vif, iph->saddr) || is_blocked_ip(vif, iph->daddr)) {
      //  printk(KERN_INFO "Dropped ingress packet for blocked IP\n");
      //  return NF_DROP;
    //}
